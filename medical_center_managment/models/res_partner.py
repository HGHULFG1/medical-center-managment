...
class DoctorTimesheet(models.Model):
    ...
    adress_id = fields.Many2one('res.partner', string="Adress", required=True,
                                domain="[('partner_type','in',['hospital','center','clinic'])]")

    def _compute_name(self):
        for record in self:
            record.name = record.adress_id.name + \
                '(' + record.dayofweek + ', ' + record.day_period + ')'
...
class DeseasseLevelPartner(models.Model):
    ...
    partner_id = fields.Many2one(
        'res.partner', string='Patient', required=True, domain="[('partner_type','=','dr')]")
    desease_id = fields.Many2one('desease', string='Desease', required=True)
    level_id = fields.Many2one('desease.level', string="Level")
    doctor_id = fields.Many2one(
        "res.partner", required=True, string="Doctor", domain="[('partner_type','=','dr')]")
    _sql_constraints = [
        ('deases_level_partner_unique',
         'unique (partner_id,desease_id)', 'Duplication in diseases')
    ]
...
class Desease(models.Model):
    ...
    level_ids = fields.Many2many(
        'desease.level', 'desease_deasease_level_rel', string='Levels')
    doctor_speciality_id = fields.Many2one(
        "doctor.speciality", string="Doctor Speciality", help="What speciality shoulf a doctor have to deal with this desease")
...
class ResPartner(models.Model):
    ...
    medical_center_ids = fields.Many2many("res.partner", "doctor_center_rel", "doctor_id",
                                          "center_id", string="Medical Centers", domain="[('partner_type','=','center')]")
    ...
    # doctor fields
    doctor_surgery_ids = fields.One2many(
        'surgery', 'doctor_id', string="Planned Surgeries")
    emergency_phone = fields.Char("Emergency Phone")
    patient_count = fields.Integer(
        "Patients", compute="_compute_patient_count")
    doctor_appiontment_ids = fields.One2many(
        'doctor.appointment', 'doctor_id', string="Appointments")
    doctor_appointment_count = fields.Integer(
        'Appointments', compute="_compute_doctor_appointment")
    meeting_product_id = fields.Many2one('product.product', string="Product For Meetings",
                                         help="This product will be used to generate invoices for meetings")
    comment = fields.Text("Notes", translate=True)
    # Patient fields
    surgery_ids = fields.One2many('surgery', 'patient_id', string='Surgeries')
    contagious_disease = fields.Boolean(
        "Contagious Diseases", compute="_compute_contagious")
    patient_appiontment_ids = fields.One2many(
        'doctor.appointment', 'patient_id', string="Appointments")
    patient_appointment_count = fields.Integer(
        'Appointments', compute="_compute_patient_appointment_count")
    disease_ids = fields.One2many(
        'desease.level.doctor.partner', 'partner_id', string="Deseases")
    doctor_count = fields.Integer('Doctors', compute="_compute_doctor_count")
    medical_ids = fields.One2many(
        "patient.medical.scheduel.scheduel", "patient_id", string="Medicals")
    medical_count = fields.Integer(
        "Medicals", compute="_compute_medical_count")
    timesheet_ids = fields.One2many(
        "doctor.timesheet", "doctor_id", string="Timesheet")
    smoker = fields.Boolean("Smoker")
    drinker = fields.Boolean("Drinker")
    measurement_ids = fields.One2many(
        'medical.patient.measure', 'patient_id', string='Measurements')
    ...
    ibw = fields.Float("Ideal Body Weight", compute="_compute_ibw",
                       help="Perfect body weight, depending on the calculation method you've specified in the settings")
    abw = fields.Float("Adjusted Body Weight", compute="_compute_abw",
                       help="Perfect body weight, depending on the calculation method you've specified in the settings")
    ...
    def _compute_ibw(self):
        ...
        for partner in self:
            if partner.gender and partner.partner_type == 'patient':
                partner.ibw = (partner.height / 30.48 - 5) * 2.3 * \
                    12 + gender_switcher.get(partner.gender)
            else:
                partner.ibw = 0.0
    ...
    def _compute_abw(self):
        for partner in self:
            if partner.weight > 1.3 * partner.ibw and partner.gender and partner.partner_type == 'patient':
                partner.abw = partner.ibw + 0.4 * \
                    (partner.weight - partner.ibw)
            else:
                partner.abw = 0.0
    ...
    def _compute_patient_appointment_count(self):
        for record in self:
            if record.partner_type != 'patient':
                ...
                record.patient_appointment_count = len(
                    record.patient_appiontment_ids.ids)
    ...
    def _compute_doctor_appointment(self):
        for record in self:
            if record.partner_type != 'dr':
                ...
                record.doctor_appointment_count = len(
                    record.doctor_appiontment_ids.ids)
    ...
    def _compute_same_name(self):
        for partner in self:
            ...
            partner.same_name = bool(
                partner.name) and self.env['res.partner'].search(domain, limit=1)
    ...
    def _compute_patient_count(self):
        for record in self:
            if record.partner_type == 'patient':
                ...
                record.patient_count = len(self.env["desease.level.doctor.partner"].search([
                                           ('doctor_id', '=', record.id)]).ids)
    ...
    def _compute_concurrency(self, from_datetime, to_datetime, addresse):
        ...
        # appoints_ids = self
        appointment_ids = self.with_context(
            tz=self.env.user.tz, lang=self.env.user.lang).doctor_appiontment_ids
        concurrent_meetings = list(filter(lambda meeting:
                                          meeting.state not in [
                                              'draft', 'done', 'cancel']
                                          and
...
# patient related functions
    ...
    def _check_side_effects_medicaments(self, medicament):
        ...
        side_effects_could_affects_patient = list(filter(
            lambda side_effect:
            not side_effect.disease_id or side_effect.disease_id.id in side_effects_ids.ids, side_effects_ids
        ))
