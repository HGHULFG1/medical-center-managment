...
class Prescriptions(models.Model):
    ...
    appointment_id = fields.Many2one(
        "doctor.appointment", string="Appointment")
    patient_id = fields.Many2one(
        "res.partner", domain="[('partner_type','=','patient')]", required=True, string="Patient")
    doctor_id = fields.Many2one(
        "res.partner", domain="[('partner_type','=','dr')]", required=True, string="Doctor")
    medical_schedueled_ids = fields.One2many(
        "patient.medical.scheduel.scheduel", "prescription_id", string="Medicals")
    name = fields.Char('Name', required=True,
                       placeholder='Prescription for sugar')

    def _default_patient_medicals(self):
        for rec in self:
            rec.patient_medical_ids = [(6, 0, rec.patient_id.medical_ids.ids)]
    patient_medical_ids = fields.Many2many(
        "patient.medical.scheduel.scheduel", string="Patient Medicals", default=_default_patient_medicals)
