...
class Medicals(models.Model):
    ...
    side_effect_ids = fields.One2many(
        "patient.medicals.side.effect", 'medical_id', string="Side Effects")
    disease_ids = fields.Many2many(
        'desease', 'desease_medical_rel', 'desease_id', 'medical_id', string="Diseases")
...
class MedicalSchedule(models.Model):
    ...
    schedule_line_ids = fields.One2many(
        'patient.medicals.scheduel.line', 'scheduel_id', string="Scheduels")
...
class MedicalScheduleLine(models.Model):
    ...
    scheduel_type = fields.Selection(
    ...
    scheduel_activity = fields.Selection([("before_breakfast", "Before Breakfast"), ("after_breakfast", "After Breakfast"), (
        "before_lunch", "Before Lunch"), ("after_lunch", "After Lunch"), ("before_dinner", "Before Dinner"), ("after_dinner", "After Dinner")])
...
class MedicalSchedulePatient(models.Model):
    ...
    medical_id = fields.Many2one(
        "patient.medicals", string="Medical", required=True)
    scheduel_id = fields.Many2one(
        "patient.medicals.scheduel", string="Scheduel", required=True)
    schedule_appointment = fields.Many2one(
        "doctor.appointment", string="Schedueld In")
    appointment_id = fields.Many2one("doctor.appointment", string="Schedueld Date",
                                     domain="[('patient_id', ' = ', patient_id), ('doctor_id', ' = ', doctor_id)]")
    doctor_id = fields.Many2one(
        "res.partner", string="Doctor", domain="[('partner_type', ' = ', 'dr')]")
    prescription_id = fields.Many2one(
        "doctor.prescription", string="Presciption")
    ...
    def _validate_medicament_for_patient(self):
        ...
        if self.patient_id.id and self.medical_id.id:
            ...
            side_effects_result = self.patient_id._check_side_effects_medicaments(
                self.medical_id)
